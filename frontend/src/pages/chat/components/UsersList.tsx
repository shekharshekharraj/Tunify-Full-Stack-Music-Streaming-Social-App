        ...
        <ScrollArea className='h-[calc(100vh-280px)]'>
          <div className='space-y-2 p-4'>
            {isLoading ? (
              ...
              users.map((user) => {
                const unreadCount = unreadCounts.get(user.clerkId) || 0;
                return (
                  <div
                    key={user._id}
                  ...
                  >
                    ...
                    {unreadCount > 0 && (
                      <div className='bg-emerald-500 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center'>
                        {unreadCount}
                      </div>
                    )}
                  </div>
